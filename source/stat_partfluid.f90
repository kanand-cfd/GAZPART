...
real(kind=8), dimension(ndim, 1) :: FFLUID, XCP, principal_axis
...
real(kind=8) :: MINAX, TAUPF, FBETA, INVTAUP
!!==================================================================================!!
...
do J = 1, NIG
    ...
    do I = 1, NPART_LOC(J)
        ...
        principal_axis(1,1) = 1.0; principal_axis(2,1) = 0.0; principal_axis(3,1) = 0.0
        ...
        ! Pitching Torque using Center of Pressure
        !XCP(1,1) = 0.25*(2.0*EMAJ_PART(J, IDP))*(1 - (sin(phi))**3.0)
        !XCP(2,1) = 0.0 ; XCP(3,1) = 0.0

        !TORQUE_PITCHING(1,1) = 0.0

        !TORQUE_PITCHING(2,1) = - XCP(1,1)*(FDRAG(3,1) + FLIFT(3,1))
        !TORQUE_PITCHING(3,1) =   XCP(1,1)*(FDRAG(2,1) + FLIFT(2,1))
        ...
        TORQUE_ROTATION(1,1) = CTROTATION * (RHOF/PMASS)/2.0 * (dequiv/2.0)**5 * abs(OREL(1,1)) * OREL(1,1)

        TORQUE_ROTATION(2,1) = CTROTATION_2 * (RHOF/PMASS)/2.0 * (dequiv/2.0)**5 * OREL_YZ * OREL(2,1)

        TORQUE_ROTATION(3,1) = CTROTATION_2 * (RHOF/PMASS)/2.0 * (dequiv/2.0)**5 * OREL_YZ * OREL(3,1)
        ...
        MEAN_PARTFLUID_LOC(26,J,IDP) = MEAN_PARTFLUID_LOC(26, J, IDP) + principal_axis(1,1)

        MEAN_PARTFLUID_LOC(27,J,IDP) = MEAN_PARTFLUID_LOC(27, J, IDP) + principal_axis(2,1)

        MEAN_PARTFLUID_LOC(28,J,IDP) = MEAN_PARTFLUID_LOC(28, J, IDP) + principal_axis(3,1)      


        !! INVTAUP when only Considering Drag !!
        INVTAUP = (0.5*RHOF*CDRAG*(PPI/4.0)*(dequiv**2)*UFLU_mag)/PMASS


        MEAN_PARTFLUID_LOC(29,J,IDP) = MEAN_PARTFLUID_LOC(29, J, IDP) + INVTAUP

        MEAN_PARTFLUID_LOC(30,J,IDP) = MEAN_PARTFLUID_LOC(30,J,IDP) + PART(I,J)%UP

        MEAN_PARTFLUID_LOC(31,J,IDP) = MEAN_PARTFLUID_LOC(31,J,IDP) + PART(I,J)%VP

        MEAN_PARTFLUID_LOC(32,J,IDP) = MEAN_PARTFLUID_LOC(32,J,IDP) + PART(I,J)%WP

        MEAN_PARTFLUID_LOC(33,J,IDP) = MEAN_PARTFLUID_LOC(33,J,IDP) + PART(I,J)%OMEGAX

        MEAN_PARTFLUID_LOC(34,J,IDP) = MEAN_PARTFLUID_LOC(34,J,IDP) + PART(I,J)%OMEGAY

        MEAN_PARTFLUID_LOC(35,J,IDP) = MEAN_PARTFLUID_LOC(35,J,IDP) + PART(I,J)%OMEGAZ

        MEAN_PARTFLUID_LOC(36,J,IDP) = MEAN_PARTFLUID_LOC(36, J, IDP) + 1.0
...
!        write(*,*) 'OXP = ', PART(I,J)%OMEGAX, ' OYP = ', PART(I,J)%OMEGAY, ' OZP = ', PART(I,J)%OMEGAZ 
    ...
    call RSUMCPU(MEAN_PARTFLUID_LOC(36,J,IDP),NPM(IDP))
...
do J = 1, NIG
    ...
    do IDP = 1, POLYDISP(J)
      ...
      MEAN_PARTFLUID(36,J,IDP) = NPM(IDP)
    ...
    do IDP = 1, POLYDISP(J)
      ...
      write(NUMFILE,10000)   &
        TIME, MEAN_PARTFLUID( 1,J,IDP),  & !- Rep
              ...
              MEAN_PARTFLUID(26,J,IDP),  & !-principal_axis(1,1)    
              MEAN_PARTFLUID(27,J,IDP),  & !-principal_axis(2,1)
              MEAN_PARTFLUID(28,J,IDP),  & !-principal_axis(3,1)

              MEAN_PARTFLUID(29,J,IDP),  & !-INVTAUP

              MEAN_PARTFLUID(30,J,IDP),  & !-UP 
              MEAN_PARTFLUID(31,J,IDP),  & !-VP 
              MEAN_PARTFLUID(32,J,IDP),  & !-WP 

              MEAN_PARTFLUID(33,J,IDP),  & !-OXP  
              MEAN_PARTFLUID(34,J,IDP),  & !-OYP
              MEAN_PARTFLUID(35,J,IDP),  & !-OZP 

              MEAN_PARTFLUID(36,J,IDP)     !- <Np>
...
!!======================================================================
if(STAT_TIME) then
 MEAN_TIME_PARTFLUID = MEAN_PARTFLUID
end if 
